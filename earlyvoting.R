...
# individual voter by election
voterpivot <- voterhistory %>%
  filter(Election_Type == "GEN") %>% 
  arrange(Elex_Date) %>%
  group_by(simple_ballot,Election_Type,CountyId,Voter_ID) %>% 
  dplyr::summarize(total=n()) %>% 
  pivot_wider(id_cols = c("County", "Voter_ID",total),
              names_from = c("Election_Type"),
              values_from = simple_ballot)

#ballot type by race over time
ballot_by_race <- voters %>% 
  group_by(County.x,across(starts_with("GEN_"))) %>% 
  summarize(total=n()) %>% 
  arrange(desc(total))

BALLOT_BY_COUNTY <- voters %>% 
  group_by(CountyId,County) %>% 
  summarize(total=n()) %>% 
  arrange(desc(total))


ballot_by_race.df = ldply(ballot_by_race, function(t) t$toDataFrame())

#voters  by party and county
voters_by_party <- voters %>% 
  group_by(simple_party,CountyId) %>% 
  dplyr::summarize(total=n()) %>% 
  arrange(desc(total))

voters_by_reg <- voters %>% 
  group_by(Reg_Date) %>% 
  dplyr::summarize(total=n()) %>% 
  arrange(desc(total))
...
voters %>%
 ggplot() +
 aes(x = age, fill = Generation) +
 geom_histogram() +
 scale_fill_hue() +
 theme_minimal() +
  labs(title = "In-Person Early Votes by Age") +
  
#Graph of voters by age
voters %>%
 ...
 labs(title = "In-Person Early Voting by Generation") +
 theme_minimal()
...
#Graph of voters by party
voters %>%
  filter(!(Generation %in% "")) %>%
  ggplot() +
  aes(x = simple_party, fill = simple_party) +
  geom_bar() +
  scale_fill_manual(values=c("#56B4E9","#006600","brown")) +
  labs(title = "In-Person Early Voting by Party") +
  theme_minimal()
...
voters %>%
  ...
  labs(title = "In-Person Early Voting by Race") +
  theme_minimal()+
...
#graph of voters by SD
voters %>%
  filter(!(Generation %in% "")) %>%
  ggplot() +
  aes(x = SenateDistrict, fill = Generation, label=SenateDistrict) +
  geom_bar() +
  scale_fill_brewer(palette = "Set1") +
  labs(title = "In-Person Early Voting by Senate District") +
  theme_minimal()


ballot_by_county <- voterhistory %>% 
  group_by(Elex_Date,ballot,Election_Type,County) %>% 
  dplyr::summarize(total=dplyr::n()) %>% 
  arrange(desc(total))

ballot_by_race_by_county <- voters %>% 
  group_by(Election_Type,ballot,Race,County.x) %>% 
  dplyr::summarize(total=n()) %>% 
  arrange(desc(total))

ballot_by_party_by_county <- voters %>% 
  group_by(Elex_Date,Election_Type,ballot,simple_party,County.y) %>% 
  summarize(total=n()) %>% 
  arrange(desc(total))

ballot_by_county <- voters %>% 
  group_by(Elex_Date,ballot,County)

ballot_by_age_party_race <- voters %>% 
  group_by(Election_Type,ballot,Race,simple_party,Generation,County.x) %>% 
  dplyr::summarize(total=n()) %>% 
  arrange(desc(total))

ballot_by_sex_race_age_party_race <- voters %>% 
  filter(grepl("PRI",Election_Type,ignore.case=TRUE)) %>% 
  group_by(Sex,Race,simple_party,Generation,ballot,County.x) %>% 
  dplyr::summarize(total=n()) %>% 
  arrange(desc(total))

ballot_by_elections_congress <- full_voters %>% 
  group_by(elections,ballot,County.x,Generation,Race,Party,County,Congressional_District) %>% 
  dplyr::summarize(total=n()) %>% 
  arrange(desc(total))


badvotes <- voters %>% 
  filter(ballot == "VbM Not Counted")